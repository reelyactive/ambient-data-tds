...
function handleAmbientData(raddec, dynamb, instance) {
  ...
  if(isTemperatureSensor) {
    ...
    insertRow(instance.connection, config.database.sqlTableName, row,
              instance.isDebug, (err) => {
      if(err) {
        handleError(err, 'Database write failed');
 ...
 * @param {Boolean} isDebug Whether to output to the console or not.
 * @param {function} callback The function to call on completion.
 */
function insertRow(connection, tableName, row, isDebug, callback) {
  let values = "('" + row.id + "'" +
               ", '" + row.deviceSignature + "'" +
               ", '" + row.timestamp + "'" +
               ", '" + row.temperature + "'" +
               ...
               ", '" + row.rssi + "')";
  let requestString = "INSERT INTO " + tableName + " VALUES " + values + ";";
  ...
  let request = new Request(requestString, callback);
